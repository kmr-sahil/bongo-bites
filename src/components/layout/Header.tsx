...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* Main Header */}
      <header
        className={`sticky top-0 z-50 bg-background transition-all duration-300 ${isScrolled ? "shadow-md" : "border-b border-border"}`}
      >
                ...
                <nav className="p-4">
                  {navLinks.map((link) => (
                    <SheetClose asChild key={link.path}>
                      <Link
                        to={link.path}
                        className={`block py-3 px-4 rounded-lg text-lg transition-colors ${location.pathname === link.path ? "bg-primary/10 text-primary font-medium" : "hover:bg-secondary"}`}
                      >
                        {link.name}
                      </Link>
                    </SheetClose>
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {navLinks.slice(0, 3).map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${location.pathname === link.path ? "text-primary" : "text-foreground/80 hover:text-foreground hover:bg-secondary/50"}`}
                >
                  {link.name}
                </Link>
              ))}
